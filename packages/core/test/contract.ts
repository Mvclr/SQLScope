...
import { toDbml, toDdl } from '../src/export/index.js';
...
export function describeEngineContract(
  ...
  describe(`privileges contract — ${engine}`, () => {
    let current: TestDatabase | undefined;
    ...
    it('reads grants to a role and to PUBLIC', async () => {
      const db = await fresh(
        `drop role if exists sqlscope_analyst`,
        `create role sqlscope_analyst`,
        `create table documents (id int)`,
        `grant select on documents to sqlscope_analyst`,
        `grant select, insert on documents to public`,
      );

      try {
        const { tableGrants, roles } = await readPrivileges(db);

        expect(tableGrants.map((g) => [g.name, g.grantee, g.privilege])).toEqual(
          expect.arrayContaining([
            ['documents', 'PUBLIC', 'INSERT'],
            ['documents', 'PUBLIC', 'SELECT'],
            ['documents', 'sqlscope_analyst', 'SELECT'],
          ]),
        );
        // The owner keeps its own privileges, listed once the table has any explicit grant.
        expect(tableGrants.every((g) => g.owner !== '')).toBe(true);
        expect(roles.map((r) => r.name)).toContain('sqlscope_analyst');
        expect(roles.find((r) => r.name === 'sqlscope_analyst')).toMatchObject({
          superuser: false,
          canLogin: false,
        });
      } finally {
        await run(db, `drop table documents`, `drop role if exists sqlscope_analyst`);
      }
    });
    ...
    it('reads row-level policies with their expressions', async () => {
      const db = await fresh(
        `create table documents (id int, organization_id int)`,
        `alter table documents enable row level security`,
        `create policy tenant_isolation on documents for select
  ...
  describe(`schema export contract — ${engine}`, () => {
    const databases: TestDatabase[] = [];

    const open = async (...statements: string[]) => {
      const database = await createDatabase();
      databases.push(database);
      await run(database.db, ...statements);
      return database.db;
    };

    afterEach(async () => {
      for (const database of databases.splice(0)) await database.dispose().catch(() => undefined);
    });
    ...
    const schema = [
      ...
      `create unique index orders_open on orders (user_id) where total > 0`,
    ...
    it('rebuilds an equivalent schema from the DDL it generates', async () => {
      const source = await introspect(await open(...schema));
    ...
    it('describes tables, keys and relationships in DBML', async () => {
      const dbml = toDbml(await introspect(await open(...schema)));
...
/** Statements generated by `toDdl` are separated by a blank line. */
const splitStatements = (ddl: string) =>
  ddl
    .split(/\n\s*\n/)
